...
_STRINGS: Dict[str, Dict[str, str]] = {
    ...
    'ru': {
        ...
        'choose_count': 'Выберите количество вопросов кнопками.',
        ...
        'count_invalid': 'Пожалуйста, выберите количество вопросов кнопками.',
...
_STRING_OVERRIDES: Dict[str, Dict[str, str]] = {
    ...
    "tr": {
        "choose_count": "Soru sayısını düğmelerden seçin.",
        ...
        "count_invalid": "Lütfen soru sayısını düğmelerden seçin.",
    ...
    "kk": {
        "choose_count": "Сұрақ санын түймелер арқылы таңдаңыз.",
        ...
        "count_invalid": "Сұрақ санын түймелер арқылы таңдаңыз.",
    ...
    "ar": {
        "choose_count": "اختر عدد الأسئلة باستخدام الأزرار.",
        ...
        "count_invalid": "يرجى اختيار عدد الأسئلة باستخدام الأزرار.",
    ...
    "ko": {
        ...
        "count_invalid": "버튼으로 문제 수를 선택해 주세요.",
...
_FILE_AND_ERROR_OVERRIDES: Dict[str, Dict[str, str]] = {
    ...
    "ru": {
        "upload_hint": "Отправьте файл (.pdf / .docx / .pptx / .xlsx / .txt).\nДля PDF/PPTX выбор диапазона страниц обязателен.",
        "upload_hint_noai": "Отправьте файл с тестом (.txt / .json / .docx / .pptx / .pdf / .xlsx).\nВ файле должны быть готовые вопросы: вопрос + 4 варианта.\nДля XLSX порядок столбцов: Вопрос | Правильный ответ | Неправильный | Неправильный | Неправильный.",
        "file_type_only": "Отправьте только файл .pdf, .docx, .pptx, .xlsx, .png, .jpg, .txt, .md или .json.",
        "err_too_many_questions": "Слишком много вопросов.\nПожалуйста, уменьшите количество вопросов и диапазон страниц и попробуйте снова.",
    },
    "en": {
    ...
    "tr": {
        ...
        "upload_hint_noai": "Bir test dosyası gönderin (.txt / .json / .docx / .pptx / .pdf / .xlsx).\nDosyada hazır sorular bulunmalıdır: soru + 4 seçenek.\nXLSX için sütun düzeni: Soru | Doğru cevap | Yanlış | Yanlış | Yanlış.",
        "file_type_only": "Lütfen yalnızca .pdf, .docx, .pptx, .xlsx, .png, .jpg, .txt, .md veya .json dosyası gönderin.",
        "err_too_many_questions": "Soru sayısı çok fazla.\nLütfen soru sayısını ve sayfa aralığını azaltıp tekrar deneyin.",
    },
    "kk": {
        "upload_hint": "Файл жіберіңіз (.pdf / .docx / .pptx / .xlsx / .txt).\nPDF/PPTX үшін бет аралығын таңдау міндетті.",
        "upload_hint_noai": "Тест файлын жіберіңіз (.txt / .json / .docx / .pptx / .pdf / .xlsx).\nФайлда дайын сұрақтар болуы керек: сұрақ + 4 нұсқа.\nXLSX үшін баған реті: Сұрақ | Дұрыс жауап | Қате | Қате | Қате.",
        "file_type_only": "Тек .pdf, .docx, .pptx, .xlsx, .png, .jpg, .txt, .md немесе .json файлын жіберіңіз.",
        "err_too_many_questions": "Сұрақ саны тым көп.\nСұрақ санын және бет аралығын қысқартып, қайта көріңіз.",
    },
    "ar": {
        "upload_hint": "أرسل ملفًا (.pdf / .docx / .pptx / .xlsx / .txt).\nبالنسبة إلى PDF/PPTX يجب اختيار نطاق الصفحات.",
        "upload_hint_noai": "أرسل ملف اختبار (.txt / .json / .docx / .pptx / .pdf / .xlsx).\nيجب أن يحتوي الملف على أسئلة جاهزة: سؤال + 4 خيارات.\nفي XLSX يكون ترتيب الأعمدة: السؤال | الإجابة الصحيحة | خطأ | خطأ | خطأ.",
        "file_type_only": "يرجى إرسال ملف بصيغة .pdf أو .docx أو .pptx أو .xlsx أو .png أو .jpg أو .txt أو .md أو .json فقط.",
        "err_too_many_questions": "عدد الأسئلة كبير جدًا.\nيرجى تقليل عدد الأسئلة ونطاق الصفحات ثم المحاولة مرة أخرى.",
    },
    "zh": {
        "upload_hint": "发送文件 (.pdf / .docx / .pptx / .xlsx / .txt)。\nPDF/PPTX 必须选择页码范围。",
        "upload_hint_noai": "发送测验文件 (.txt / .json / .docx / .pptx / .pdf / .xlsx)。\n文件应包含现成题目：题目 + 4 个选项。\nXLSX 列顺序：题目 | 正确答案 | 错误 | 错误 | 错误。",
        "file_type_only": "请仅发送 .pdf、.docx、.pptx、.xlsx、.png、.jpg、.txt、.md 或 .json 文件。",
        "err_too_many_questions": "题目数量过多。\n请减少题目数量和页码范围后重试。",
    },
    "ko": {
        "upload_hint": "파일을 보내세요 (.pdf / .docx / .pptx / .xlsx / .txt).\nPDF/PPTX의 경우 페이지 범위를 반드시 선택해야 합니다.",
        "upload_hint_noai": "퀴즈 파일을 보내세요 (.txt / .json / .docx / .pptx / .pdf / .xlsx).\n파일에는 이미 문항이 있어야 합니다: 질문 + 4개 선택지.\nXLSX 열 순서: 질문 | 정답 | 오답 | 오답 | 오답.",
        "file_type_only": ".pdf, .docx, .pptx, .xlsx, .png, .jpg, .txt, .md 또는 .json 파일만 보내주세요.",
        "err_too_many_questions": "문항 수가 너무 많습니다.\n문항 수와 페이지 범위를 줄인 뒤 다시 시도해 주세요.",
    },
}
